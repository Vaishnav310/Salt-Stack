...
import { MinionManagementPage } from "./pages/MinionManagement.js";
...
import { SaltPillarsPage } from "./pages/SaltPillars.js";
import { SaltStatesPage } from "./pages/SaltStates.js";
...
export class Router {
  ...
  constructor() {
    ...
    this._registerPage(Router.minionManagementPage = new MinionManagementPage(this));
    ...
    this._registerPage(Router.saltPillarsPage = new SaltPillarsPage(this));
    this._registerPage(Router.saltStatesPage = new SaltStatesPage(this));
    ...
    this._registerPage(Router.filesPage = new FilesPage(this));
  ...
  _registerRouterEventListeners() {
    ...
    this._registerMenuItem(null, "Minion Management", "minionmanagement", "m");
    ...
    // Removed Salt Pillars and Salt States as they are now in the Files tab
    ...
    this._registerMenuItem(null, "files", "files", "f");
  ...
  static _showMenuItem(pPages, pPage, pChildren = []) {
    ...
    for (let nr = 1; nr <= 2; nr++) {
      ...
      item.style.color = !visible && hasVisibleChild ? "lightgray" : "";
      if (!visible) {
  ...
  static updateMainMenu() {
    ...
    Router._showMenuItem(pages, Router.saltPillarsPage);
    Router._showMenuItem(pages, Router.saltStatesPage);
    ...
    Router._showMenuItem(pages, Router.filesPage);
  ...
  static _showPage(pPage) {
    ...
    // hide header, fab, docu, and warning when on the login page
    // so unauthenticated users only see the login form
    const header = document.getElementById("header");
    const popupRunCommand = document.getElementById("popup-run-command");
    if (pPage.path === "login") {
      if (header) {
        header.style.display = "none";
      }
      if (popupRunCommand) {
        popupRunCommand.style.display = "none";
      }
    } else {
      if (header) {
